...
本仓库包含一个 [`@aleabitoreddit`](https://x.com/aleabitoreddit) 的 partial mixed-source 示例分析，采集于 2026-06-14，来源是当前本地可用 CDP/GraphQL 和 DOM captures 的最佳 union：
...
重要边界：只有 run summary 中 `completeness.isFullHistory: true` 的报告才算完整账号画像。partial mixed-source 和 DOM-only 报告只能作为验证 artifact，除非 full-history gate 通过。完整 posts/replies 分析需要来自 CDP 的 GraphQL/Relay 分页，或本地 DevTools cURL 文件。
...
## Hermes / OpenClaw / Claude Code 使用方式

Hermes、OpenClaw、Claude Code 和其他 coding agents 即使不原生支持 Codex `SKILL.md`，也可以把这个仓库当作 CLI-first skill 使用。

1. 克隆仓库，安装 Node.js 20+，先运行 `npm test`。
2. 先问用户报告语言：英文对应 `--language en`，中文对应 `--language zh`。如果没有偏好，默认用 `zh`。
3. 可发布账号报告优先走已登录 CDP 路径：

```bash
npm run chrome:cdp
# 让用户在打开的隔离 Chrome 窗口中登录 X。
npm run run -- --username <handle> --language <en|zh> --require-full-history
```

4. 如果 CDP 不可用，让用户把 Chrome DevTools `Copy as cURL` 输出保存到本地 gitignored 文件，然后运行：
...
5. 如果 agent 已经有 capture 文件，使用 `--input <capture.json>`，并在解释结果前阅读 run summary。
6. 除非 `reports/<handle>-run-summary-YYYY-MM-DD.json` 中 `completeness.isFullHistory: true`，否则不要宣称这是完整账号分析。
7. `dom-only` 或 `mixed` capture 如果状态是 `not-exhausted`、`partial-dom-only` 或 `source-exhausted-count-gap`，都应当作 partial report。没有父帖/reply 证据的行保持 `unclassified`。
8. 不要提交 `x_curl/`、`.chrome-cdp-profile/` 或被忽略的 `data/raw/*.json`。只提交明确想发布的 Markdown/CSV/run-summary 报告。
