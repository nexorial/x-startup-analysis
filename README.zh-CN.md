...
- **外部视角增长洞察**：相关性、主题、发布时间/格式、top posts、互动账号和可复制经验。
- **本地优先输出**：Markdown 和 CSV 易于检查、提交或发布。

## 示例分析

本仓库包含一个真实的 [`@aleabitoreddit`](https://x.com/aleabitoreddit) DOM-only 示例分析，采集于 2026-06-14：
...
重要边界：这个示例是 `dom-only`。它适合分析公开互动数据，但如果要精确 post/reply 比例，需要 GraphQL/Relay capture 或单帖 SSR enrichment。
